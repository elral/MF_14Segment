...
void MFCustomDevice::attach(uint16_t adrPin, uint16_t adrType, uint16_t adrConfig)
{
    ...
    ********************************************************************************** */
    ...
    if (strcmp(parameter, "Elral_14Segment") == 0)
        _customType = MY_14SEGMENT;

    if (_customType == MY_14SEGMENT) {
        /* **********************************************************************************
            Check if the device fits into the device buffer
        ********************************************************************************** */
        if (!FitInMemory(sizeof(Segment14))) {
        ...
        ********************************************************************************** */
        ...
        _my14segment = new (allocateMemory(sizeof(Segment14))) Segment14();
        _my14segment->attach(addrI2C);
...
void MFCustomDevice::set(int16_t messageID, char *setPoint)
{
    ...
    if (_customType == MY_14SEGMENT) {
        _my14segment->set(messageID, setPoint);
    }
}
